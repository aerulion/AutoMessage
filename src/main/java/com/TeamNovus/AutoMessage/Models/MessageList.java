    ...
    if (message != null) {
      ...
      for (int i = 0; i < messages.size(); i++) {
        ...
        if (message.isJsonMessage(i) && to instanceof Player) {
          ...
          try {
            // Parse the message
            Object parsedMessage = Class.forName(
                    "net.minecraft.server." + v + ".IChatBaseComponent$ChatSerializer")
                .getMethod("a", String.class)
                .invoke(null, ChatColor.translateAlternateColorCodes("&".charAt(0), m));
            ...
            Object craftPlayer = Class.forName(
                "org.bukkit.craftbukkit." + v + ".entity.CraftPlayer").cast(to);
            Object craftHandle = Class.forName(
                    "org.bukkit.craftbukkit." + v + ".entity.CraftPlayer").getMethod("getHandle")
                .invoke(craftPlayer);
