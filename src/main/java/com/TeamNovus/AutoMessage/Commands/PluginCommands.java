...
public class PluginCommands {
  ...
  public void onUpdateCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    Updater updater = new Updater(AutoMessage.plugin, 37718, AutoMessage.plugin.getFile(),
        UpdateType.NO_DOWNLOAD, false);

    if (updater.getResult() == UpdateResult.UPDATE_AVAILABLE) {
      sender.sendMessage(ChatColor.GREEN + "There is an update available! Downloading update...");

      UpdateResult result = new Updater(AutoMessage.plugin, 37718, AutoMessage.plugin.getFile(),
          UpdateType.DEFAULT, true).getResult();
      if (result == UpdateResult.SUCCESS) {
        sender.sendMessage(ChatColor.RESET + updater.getLatestName() + ChatColor.GREEN
            + " has been downloaded sucessfully!");
      } else {
        sender.sendMessage(ChatColor.RED + "There was an error downloading " + ChatColor.RESET
            + updater.getLatestName() + ChatColor.RED + "!");
      }
    } else {
      PluginDescriptionFile desc = AutoMessage.plugin.getDescription();

      sender.sendMessage(
          ChatColor.RESET + desc.getName() + " v" + desc.getVersion() + ChatColor.GREEN
              + " is up  to date!");
    }
  }
  ...
  public void onEnableCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    ...
    if (list != null) {
      ...
      sender.sendMessage(
          ChatColor.GREEN + "Enabled: " + ChatColor.YELLOW + list.isEnabled() + ChatColor.GREEN
              + "!");
    } else {
      sender.sendMessage(ChatColor.RED + "The specified list does not exist!");
    }
  ...
  public void onIntervalCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    ...
    if (list != null) {
      if (Utils.isInteger(args[1])) {
        ...
        sender.sendMessage(
            ChatColor.GREEN + "Interval: " + ChatColor.YELLOW + Integer.valueOf(args[1])
                + ChatColor.GREEN + "!");
      } else {
  ...
  public void onExpiryCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    ...
    if (list != null) {
      try {
        ...
        if (list.getExpiry() != -1) {
          sender.sendMessage(
              ChatColor.GREEN + "Expires in " + ChatColor.YELLOW + Utils.millisToLongDHMS(
                  list.getExpiry() - System.currentTimeMillis()) + ChatColor.GREEN + "!");
        } else {
  ...
  public void onRandomCmd(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    ...
    if (list != null) {
      ...
      sender.sendMessage(
          ChatColor.GREEN + "Random: " + ChatColor.YELLOW + list.isRandom() + ChatColor.GREEN
              + "!");
    } else {
      sender.sendMessage(ChatColor.RED + "The specified list does not exist!");
    }
